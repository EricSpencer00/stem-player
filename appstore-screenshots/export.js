...
const outDir = resolve(repoRoot, "appstore-screenshots", "out", "iphone");
const chromes = [
...
try {
  ...
  for (const id of ["s1", "s2", "s3", "s4"]) {
    ...
    await shot.screenshot({ path: resolve(outDir, `${id}.png`) });
  }
  console.log(`Exported iPhone screenshots to ${outDir}`);
} finally {
  await browser.close();
  await server.close();
}
